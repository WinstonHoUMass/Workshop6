...
function postComment(user, content, feedItemId) {
    var feedItem = readDocument('feedItems', feedItemId);
    var time = new Date().getTime();
    var newComment = {
        "author": user,
...
app.delete('/feeditem/:feeditemid/comments/:commentidx/likelist/:userid', function(req, res) {
    ...
    if (fromUser === userId) {
        var feedItem = readDocument('feedItems', feedItemId);
        var commentItem = feedItem.comments[req.params.commentidx];
        var likeIndex = commentItem.likeCounter.indexOf(userId);
        if (likeIndex !== -1) {
            commentItem.likeCounter.splice(likeIndex, 1);
            writeDocument('feedItems', feedItem);
        }
        commentItem.author = readDocument('users', commentItem.author);
